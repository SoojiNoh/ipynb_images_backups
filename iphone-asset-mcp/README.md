...
무료 Apple 계정으로 서명한 앱은 7일 뒤 만료됩니다. 그때마다 사람이 기억해서
명령을 치는 대신, Mac 이 6일마다 스스로 다시 설치하게 걸어 둡니다.
...
Mac 이 켜져 있고 아이폰이 같은 Wi-Fi 에 있으면 알아서 됩니다. 자고 있었으면
깨어난 직후에 밀린 작업이 실행됩니다. 실패하면 무엇이 필요한지 알림으로 뜹니다.
로그는 `~/Library/Logs/AssetBridge/`.

## 주소가 바뀌어도 유지되는 연결

`tools/link_device.sh` 는 IP 대신 `<기기이름>.local` 로 붙을 수 있는지 먼저
확인하고, 되면 그쪽으로 등록합니다. DHCP 가 IP 를 바꿔도 이름은 그대로라
다시 등록할 필요가 없습니다. mDNS 를 막는 공유기에서는 자동으로 IP 로
돌아갑니다 — 그때는 주소가 바뀔 때마다 `link_device.sh` 를 한 번 돌리면 됩니다.
